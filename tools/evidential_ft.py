...
from ib_edl.train_eval import ClassificationMetric, EvidentialTrainer, UpdateRegWeightCallback, plot_predictions
...
def main():
    ...
    trainer = EvidentialTrainer(
        ...
        callbacks=[UpdateRegWeightCallback(**cfg.edl_loss_cfg['reg_weight_cfg'])])
    ...
    for key, value in test_metrics.items():
        logger.info(f'Test metrics: {key}: {value}')
    ...
    if cfg.process_preds['npz_file'] is not None or cfg.process_preds['do_plot']:
        ...
        if cfg.process_preds['do_plot']:
            plot_predictions(predictions, cfg.porcess_preds['plot_cfg'], work_dir)
        if cfg.process_preds['npz_file'] is not None:
            save_predictions(predictions, osp.join(work_dir, 'preds', cfg.process_preds['npz_file']), logger=logger)
